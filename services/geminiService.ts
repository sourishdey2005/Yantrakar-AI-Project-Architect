...
export const generateProjectIdea = async (userInput: string): Promise<ProjectIdea> => {
  ...
  // The application should not handle API key configuration UI or logic. This resolves the TypeScript error.
...
Based on this request, generate a complete project plan.`;
  ...
  try {
    ...
    // Fix: Removed specific API key error messaging to align with guidelines.
    // The user should not be prompted about API keys. A generic error is sufficient.
    throw new Error("Failed to get a valid response from the AI model. The service may be busy or there may be a configuration issue.");
  }
};
